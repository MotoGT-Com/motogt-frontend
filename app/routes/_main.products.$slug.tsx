...
import type { Route } from "./+types/_main.products.$slug";
import { capitalizeWords, formatYearRange, mergeMeta } from "~/lib/utils";
import { href, Link, redirect, useLocation, useRevalidator } from "react-router";
...
export async function loader({ params, request, context }: Route.LoaderArgs) {
  try {
    ...
    // New canonical pattern: fetch by trailing ID, with legacy slug fallback.
    const productResponse = productId
      ? await getApiProductsPublicByProductId({
          path: {
            productId,
          },
          query: {
      ...
      : await getApiProductsPublicSlugBySlug({
          path: {
            slug: params.slug!,
          },
          query: {
    ...
    let englishProductName: string | undefined;
    if (productId) {
      const englishProductResponse = await getApiProductsPublicByProductId({
        path: {
          productId,
        },
        query: {
      ...
      englishProductName = englishProductResponse.data?.data?.translations?.[0]?.name;
    }

    const canonicalSlugSegment = buildProductSlugSegment(productResponse.data.data);
  ...
  return mergeMeta(args, [
    ...
    },
    {
      "script:ld+json": {
        ...
        image: [args.loaderData.product.mainImage ?? ""],
        offers: {
          "@type": "Offer",
          price: product.price,
          priceCurrency: "JOD",
...
export default function ProductPage({ loaderData }: Route.ComponentProps) {
  ...
  const getImageGallery = () => {
    if (currentVariant?.images && currentVariant.images.length > 0) {
      return currentVariant.mainImage
        ? [currentVariant.mainImage, ...currentVariant.images]
        : currentVariant.images;
    }
    return (product as any).secondaryImage
      ? [(product as any).secondaryImage, product.mainImage, ...product.images]
      : product.images;
  };
          ...
          <div className="space-y-4 ps-[calc(max(0px,(100vw-80rem)/2)+1.5rem)] bg-background-secondary py-10 pe-6 md:pe-10">
            ...
            <div className="relative rounded-lg">
              <img
                ...
                alt={getLocalizedTranslation(product.translations)?.name}
            ...
            <div className="flex flex-wrap gap-2 overflow-scroll">
              {imageGallery.map((image, index) => (
                <button
                  ...
                  className={`relative w-[90px] md:w-[120px] aspect-[4/3] rounded border-2 overflow-hidden ${
                    selectedImage === index
